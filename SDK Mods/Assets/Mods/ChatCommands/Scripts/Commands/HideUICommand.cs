...
{
    public class HideUICommand : IClientCommandHandler
    {
        ...
        {
            if (parameters.Length < 1) return new CommandOutput("Not enough arguments. Usage:\nhide <target> [state]", CommandStatus.Error);
        ...
        public void UpdateUIStatus()
        {
            ...
            GameObject uiCamera = Manager.ui.UICamera;
            foreach (Transform transform in uiCamera.transform)
            {
        ...
        public static void SetState(string name, bool value)
        {
            if (name.Equals("all"))
            {
                foreach (string validName in validNames) SetStateDirect(validName, value);
                return;
            }
        ...
        public static bool GetState(string name)
        {
            if (name.Equals("all"))
            {
                if (!states.ContainsKey("player")) return true;
        ...
        {
            if (validNames.Contains(target)) return true;
