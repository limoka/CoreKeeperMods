...
namespace ChatCommands.Chat.Commands
{
    public class RemoveTileAreaCommand : IServerCommandHandler
    {
        public CommandOutput Execute(string[] parameters, Entity sender)
        {
            ...
            int2 startPos = CommandUtil.ParsePos(parameters, parameters.Length - 3, transform.Position, out var commandOutput);
            if (commandOutput != null)
                return commandOutput.Value.AppendAtStart("Start pos");

            int2 endPos = CommandUtil.ParsePos(parameters, parameters.Length - 1, transform.Position, out var commandOutput1);
