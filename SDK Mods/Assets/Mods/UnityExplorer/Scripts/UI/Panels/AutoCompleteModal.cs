...
namespace UnityExplorer.UI.Panels
{
    ...
    {
        public static AutoCompleteModal Instance => UE_UIManager.GetPanel<AutoCompleteModal>(UE_UIManager.Panels.AutoCompleter);

        public override string Name => "AutoCompleter";
        public override UE_UIManager.Panels PanelType => UE_UIManager.Panels.AutoCompleter;
        ...
        {
            UE_UIManager.UiBase.Panels.OnPanelsReordered += UIPanel_OnPanelsReordered;
            UE_UIManager.UiBase.Panels.OnClickedOutsidePanels += AutoCompleter_OnClickedOutsidePanels;
        }
        ...
        public static bool CheckNavigation(ISuggestionProvider handler)
        {
            ...
            bool up = UInputManager.GetKey(KeyCode.UpArrow);
            bool down = UInputManager.GetKey(KeyCode.DownArrow);

            if (up || down)
            {
                if (up)
                {
                    if (UInputManager.GetKeyDown(KeyCode.UpArrow))
                    {
                ...
                {
                    if (UInputManager.GetKeyDown(KeyCode.DownArrow))
                    {
                        SetSelectedSuggestion(SelectedIndex + 1);
                        timeOfLastNavHold = Time.realtimeSinceStartup + 0.3f;
                    }
        ...
        {
            return Suggesting(handler) && UInputManager.GetKeyDown(KeyCode.Return);
        }

        public static bool CheckEscape(ISuggestionProvider handler)
        {
            return Suggesting(handler) && UInputManager.GetKeyDown(KeyCode.Escape);
        }
        ...
        private void UIPanel_OnPanelsReordered()
        {
            ...
            if (this.UIRoot.transform.GetSiblingIndex() != UE_UIManager.UiBase.Panels.PanelHolder.transform.childCount - 1)
            {
                if (CurrentHandler != null)
                    ReleaseOwnership(CurrentHandler);
                else
